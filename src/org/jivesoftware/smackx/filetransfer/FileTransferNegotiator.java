...
public class FileTransferNegotiator {
     ...
     */
    public static FileTransferNegotiator getInstanceFor(final XMPPConnection connection) {
        if (connection == null) {
     ...
     */
    public static void setServiceEnabled(final XMPPConnection connection, final boolean isEnabled) {
     ...
     */
    public static IQ createIQ(final String ID, final String to, final String from, final IQ.Type type) {
        IQ iqPacket = new IQ() {
    ...
    private StreamNegotiator getNegotiator(final FormField field)
        ...
        boolean isIBB = false;
        for (Iterator<Option> it = field.getOptions(); it.hasNext();) {
    ...
    private StreamNegotiator getOutgoingNegotiator(final FormField field) throws XMPPException {
        ...
        if (isByteStream && isIBB) {
            return new FaultTolerantNegotiator(connection, byteStreamTransferManager.createNegotiator(), inbandTransferManager);
        }
        else if (isByteStream) {
            return byteStreamTransferManager.createNegotiator();
        }
...
//        field.setType(FormField.TYPE_LIST_MULTI); //TODO!

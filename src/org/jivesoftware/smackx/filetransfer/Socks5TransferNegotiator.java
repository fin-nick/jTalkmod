...
public class Socks5TransferNegotiator extends StreamNegotiator {
    ...
    public static boolean isAllowLocalProxyHost = false;
    private final XMPPConnection connection;
    private Socks5TransferNegotiatorManager transferNegotiatorManager;

    public Socks5TransferNegotiator(Socks5TransferNegotiatorManager transferNegotiatorManager, final XMPPConnection connection)
    {
    ...
    InputStream negotiateIncomingStream(Packet streamInitiation) throws XMPPException {
        ...
        Bytestream streamResponse = createUsedHostConfirmation(selectedHost.selectedHost, streamHostsInfo.getFrom(), streamHostsInfo.getTo(), streamHostsInfo.getPacketID());
    ...
    private SelectedHostInfo selectHost(Bytestream streamHostsInfo) throws XMPPException {
        ...
        while (it.hasNext()) {
            ...
            try {
                ...
                establishSOCKS5ConnectionToProxy(socket, createDigest(streamHostsInfo.getSessionID(), streamHostsInfo.getFrom(), streamHostsInfo.getTo()));
                break;
            }
            catch (Exception e) {
                e.printStackTrace();
            ...
            final Bytestream query) throws XMPPException, IOException
    {
        ...
        PacketCollector collector = connection.createPacketCollector(new PacketIDFilter(query.getPacketID()));

...
public class MultiUserChat {
     ...
     *      409 error can occur if someone is already in the group chat with the same nickname.
     */
    public synchronized void join(String nickname, String password, DiscussionHistory history, long timeout, Presence joinPresence)
        throws XMPPException {
        if (nickname == null || nickname.equals("")) {
            throw new IllegalArgumentException("Nickname must not be null or blank.");
        }
        ...
        if (joinPresence == null) joinPresence = new Presence(Presence.Type.available);
        joinPresence.setTo(room + "/" + nickname);
    ...
    private Collection<Affiliate> getAffiliatesByOwner(String affiliation) throws XMPPException {
        ...
        Iterator<MUCOwner.Item> it = answer.getItems();
        while (it.hasNext()) {
    ...
    private Collection<Affiliate> getAffiliatesByAdmin(String affiliation) throws XMPPException {
        ...
        // Get the list of affiliates from the server's answer
        List<Affiliate> affiliates = new ArrayList<Affiliate>();
        Iterator<MUCAdmin.Item> it = answer.getItems();
        while (it.hasNext()) {
            affiliates.add(new Affiliate(it.next()));
        }
        return affiliates;
    ...
    */
            ...
            public void processPacket(Packet packet) {
                ...
                if (presence.getType() == Presence.Type.available) {
                    ...
                    if (oldPresence != null) {
                        ...
                        boolean isAffiliationModification = false;
                        boolean isRoleModification = checkRoleModifications(oldRole, newRole, isUserStatusModification, from);
                        if (!isRoleModification) 
                        	isAffiliationModification = checkAffiliationModifications(oldAffiliation, newAffiliation, isUserStatusModification, from);
                        
                        if (!isRoleModification && !isAffiliationModification) {
                        	List<String> params = new ArrayList<String>();
                            params.add(from);
                        	fireParticipantStatusListeners("statusChanged", params);
                        }
                    }
                    else {
     ...
     */
    private boolean checkRoleModifications(String oldRole, String newRole, boolean isUserModification, String from) {
     ...
     * (e.g. room@conference.jabber.org/nick).
     */
    private boolean checkAffiliationModifications(String oldAffiliation, String newAffiliation, boolean isUserModification, String from) {

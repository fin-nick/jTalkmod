...
//    }
    ...
    /**
     * Returns an sorted (A->Z) collection of all entries in the roster, including entries
    ...
    public Collection<RosterEntry> getEntries() {
        ...
        List<RosterEntry> list = new ArrayList<RosterEntry>(Collections.unmodifiableCollection(allEntries));
    	if (list.size() > 1) {
    		Collections.sort(list, new Comparator<RosterEntry>() {
				public int compare(RosterEntry entry1, RosterEntry entry2) {
					String name1 = entry1.getName();
					if (name1 == null) name1 = entry1.getUser();
...
//        return Collections.unmodifiableCollection(allEntries);
    ...
    /**
     * Returns an sorted collection for the unfiled roster entries. An unfiled entry is
     * an entry that doesn't belong to any groups.
     *
     * @return the unfiled roster entries.
     */
    public Collection<RosterEntry> getUnfiledEntries() {
    	List<RosterEntry> list = new ArrayList<RosterEntry>(unfiledEntries);
    	if (list.size() > 1) {
    		Collections.sort(list, new Comparator<RosterEntry>() {
				public int compare(RosterEntry entry1, RosterEntry entry2) {
					String name1 = entry1.getName();
...
//        return Collections.unmodifiableList(unfiledEntries);
    ...
    public Collection<RosterGroup> getGroups() {
    	List<RosterGroup> list = new ArrayList<RosterGroup>(groups.values());
    	if (list.size() > 1) {
    		Collections.sort(list, new Comparator<RosterGroup>() {
				public int compare(RosterGroup entry1, RosterGroup entry2) {
					String name1 = entry1.getName();
        			String name2 = entry2.getName();
					return ((Comparable<String>) name1).compareTo(name2);
				}

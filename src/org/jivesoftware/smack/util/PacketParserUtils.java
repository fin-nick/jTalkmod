...
public class PacketParserUtils {
    ...
    public static IQ parseIQ(XmlPullParser parser, Connection connection) throws Exception {
        ...
        while (!done) {
            ...
            if (eventType == XmlPullParser.START_TAG) {
                ...
                else if (elementName.equals("query") && namespace.equals("jabber:iq:version") && type == IQ.Type.GET) {
               		iqPacket = new IQ() {
               			public String getChildElementXML() {
               				return null;
                        }
                    };
               		sendVersionInfo(from, id, connection);
                }
                else if (elementName.equals("time") && namespace.equals("urn:xmpp:time")) {
                	if (type == IQ.Type.GET) {
                		iqPacket = new IQ() {
                            public String getChildElementXML() {
                                return null;
                		...
                		sendTimeInfo(from, id, connection);
                	}
                }
                else if (elementName.equals("query") && namespace.equals("http://jabber.org/protocol/disco#info")) {
                	if (type == IQ.Type.GET) {
                		iqPacket = new IQ() {
                            public String getChildElementXML() {
                                return null;
                		...
                		sendDiscoInfo(from, id, connection);
                	} else if (type == IQ.Type.RESULT) {
                		iqPacket = new DiscoverInfoProvider().parseIQ(parser);
                	}
                }
    ...
    private static Object decode(Class type, String value) throws Exception {
  	...
  	private static void sendDiscoInfo(String to, String id, Connection connection) {
  		DiscoverInfo info = new DiscoverInfo();
  		info.setTo(to);
  		info.setPacketID(id);
  		info.setType(IQ.Type.RESULT);
  		for(String feature : connection.getFeatures()) {
  			info.addFeature(feature);
  		}
  		connection.sendPacket(info);
  	}
  	
  	private static void sendTimeInfo(String to, String id, Connection connection) {
  		Time result = new Time(Calendar.getInstance());
  		result.setTo(to);
  		result.setPacketID(id);
  		result.setType(IQ.Type.RESULT);
  		connection.sendPacket(result);
